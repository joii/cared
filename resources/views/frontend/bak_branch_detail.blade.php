                ...
                <div class="col-xl-6">
                    <div class="branch-infos">
                        ...
                        <div class="price-block">
                            <h3 class="price">{!! $branch->price_range !!}</h3>
                            <p>บาท/เดือน</p>
                        </div>
                    </div><!--branch-infos-->
                </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    @foreach ($rate_card_standard as $item)
                       @if($item->period_id==1)
                          @if($item->dormitory_room_price!="")<td>{{ $item->dormitory_room_price }}</td>@endif
                          @if($item->dormitory_room_price!="")<td>{{ $item->double_room_price }}</td>@endif
                          @if($item->dormitory_room_price!="")<td>{{ $item->single_room_price }}</td>@endif
                          @if($item->detached_house_price!="")<td>{{ $item->detached_house_price }}</td>@endif
                        @endif
                    @endforeach
                ...
                <tr>
                    ...
                    @foreach ($rate_card_standard as $item)
                       @if($item->period_id==2)
                          ...
                          @if($item->dormitory_room_price!="")<td>{{ $item->double_room_price }}</td>@endif
                          @if($item->dormitory_room_price!="")<td>{{ $item->single_room_price }}</td>@endif
                          @if($item->detached_house_price!="" || $has_detached_house==true )<td>{{ $item->detached_house_price }}</td>@endif
                        @endif
                    @endforeach
                </tr>
                ...
                <tr class="border-0">
                    <td rowspan="5" class="text-center border-0" style="background-color: #E7E0D7;">
                        <h4>Rehab</h4>
                        <p class="m-0 fs-18">*โปรแกรมฟื้นฟู<br>
                            โรคหลอดเลือดสมอง</p>
                    </td>
                ...
                @if($rehab_period[0]!=0 && $head_table_rehab !=1)
                <tr>
                    <td class="text-black">รายวัน</td>
                    @foreach ($rate_card_standard as $item)
                       @if($item->period_id==1)
...
<div class="section" data-aos="fade-in">
    <div class="container">
        ...
        <div class="buttons button-contact">
            ...
            <a class="btn btn-xl btn-green" href="tel:0655022166" target="_blank">สายด่วน Care Advisor 065-502-2166</a>
        </div>
    </div><!--container-->
</div><!--section-->
